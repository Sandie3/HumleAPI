...
router.post( '/', async ( req, res ) => {
    ...
    try {
        ...
        const { email, password } = req.body; // Pull email & pwd from request and store as vars

        const user = await User.findOne( { email: email } ); // Search for user in DB with same email from request
